...
\begin{proof}
    ...
    In figura ?? mostriamo l'inserimento nel tableau $T$ delle lettere
    \[
    ...
    Vogliamo studiare pi\`u precisamente dove avviene la deviazione sul percorso alla
    sinistra.
    ...
    \begin{lemma}
        ...
        Allora la deviazione avviene alla prima riga $r$ in cui $a_{r+1} > b_r$.
    \end{lemma}

    \begin{proof}[Dimostrazione del lemma]
        Prima di ogni inserimento, la lettera $a_{r+1}$ si trova nella riga $r$
        assieme a $b_{r+1}$.
        ...
        \[
            b_r < a_{r+1} < b_{r+1}.
        \]
        Questo assumendo non avvengano inserimenti speciali.

        % Pu\`o succedere, per via dell'inserimento speciale, che la lettera inserita
        % in una riga sia la lettera che leggiamo nel percorso \emph{meno uno}.
        % Questo non crea problemi in quanto rimane la disuguaglianza tra le lettere
        % spinte, ovvero la lettera spinta da $a_r$ \`e minore della lettera spinta
        % da $b_r$.
        % sia uno in meno della lettera 
    \end{proof}
