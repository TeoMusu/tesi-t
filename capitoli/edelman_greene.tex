...
Iniziamo richiamando alcune nozioni dal Capitolo~\ref{ch:basics}.

Consideriamo  il gruppo simmetrico $S_n$ come generato dalle trasposizioni di
elementi adiacenti $s_1, s_2, \dots, s_{n-1}$, dove $s_i = (i , i+1)$.
Una parola $v = v_1 v_2 \cdots v_k$ nell'alfabeto $\{1, 2, \dots, n-1 \}$
...
possibile tra tutte le parole associate a $\w$.
Questa lunghezza minima viene chiamata \emph{lunghezza} di $\w$, denotata con $\len(\w)$,
e si dimostra essere uguale al numero di inversioni di $\w$.
Per ogni permutazione $\w \in \sym_n$, chiamo $\rw(\w)$ l'insieme delle 
parole ridotte di $\w$.
L'obiettivo rimane trovare un modo per calcolare la cardinalit\`a di questo insieme.

In questo capitolo descriviamo la corrispondenza di Edelman--Greene (EG), 
introdotta da loro in \cite[Section~6]{edelman-greene-87} come una variante
della pi\`u famosa corrispondenza di Robinson--Schensted--Knuth (RSK),
ma specializzata e adattata all'inserimento di parole ridotte.

Cos\`\i\ come la corrispondenza RSK mappa parole in coppie di tableaux con la 
stessa forma di cui uno semistandard e l'altro standard, la corrispondenza EG
mappa \emph{parole ridotte} in coppie di tableaux con la stessa forma di
cui uno \emph{ridotto} e l'altro standard.
Iniziamo definendo i tableaux ridotti.

\begin{definition}
    Un tableau si dice \emph{ridotto} se \`e semistandard e il suo
    row-reading $\rowreading(T)$ \`e una
    parola ridotta.
...
\begin{example}
    Il tableau $T$ raffigurato sotto \`e ridotto in quanto $\rowreading(T) = 43451246$
    \`e una parola ridotta per 
    $\w = s_4 s_3 s_4 s_5 s_1 s_2 s_4 s_6= 2516473 \in \sym_7$.
...
\begin{remark}
    ...
    uguali e non sarebbe una parola ridotta.
\end{remark}
...
Siano $T$ un tableau ridotto e $k$ una lettera tale che $\rowreading(T)k$ sia ancora una
...
i_u \\
    ...
    \item \label{item:eg_ins3}
        ...
        $k+1$. In questo caso diciamo che \`e avvenuto un inserimento \emph{speciale}.
...
\begin{remark} 
    ...
    \begin{itemize}
        \item nel 
            caso di un inserimento regolare,
            % caso~\ref{item:eg_ins2},
            $k$ commuta con tutti gli $i_j$ a destra di $i_u$,
            ...
            \]
        \item nel 
            caso di un inserimento speciale,
            % caso~\ref{item:eg_ins3},
            come prima $k$ commuta con tutti gli $i_j$
...
\begin{example}
    ...
    \[ 
        \ytableaunumber{4, 3 4 5, 1 2 4 6 \none {\none[\leftarrow 3]}}
        \qquad
        \ytableaunumber{4, 3 4 5 \none {\none[\leftarrow 4]}, 1 2 {*(yellow) 3} 6 }
        \qquad
        \ytableaunumber{4 \none {\none[\leftarrow 5]}, 3  4 {*(yellow) 5}, 1 2 {*(yellow) 3} 6 }
        \qquad
        \ytableaunumber{4 {*(yellow) 5}, 3 4 {*(yellow) 5} , 1 2 {*(yellow) 3} 6 }.
    \]
    
\end{example}

\begin{definition}
    Nell'esempio sopra, ho colorato in ogni riga la casella che conteneva la lettera che \`e
    stata spinta nella riga successiva.
    In particolare, quando inserendo $k$ avviene un inserimento speciale,
    coloro la casella di $k+1$.
    Queste caselle prendono il nome di \emph{percorso di inserimento}.
\end{definition}


\begin{proposition}
    \label{prop:propriet_inserimento}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\ytableaushort 
        { \none \none {\scriptstyle >b} \none \none {\none[\leftarrow b]}, 
        {\scriptstyle <a} {\scriptstyle <a} {*(yellow) a}  }
        * {4,5}
        &&\text{ se $a < b$.}
        \\[1em]
        &\ytableaushort 
        { \none \none {\scriptstyle >a} {
                % \scriptstyle {> \atop a+1}
                \substack{> \\ a+1}
            } \none {\none[\leftarrow a+1]}, 
        {\scriptstyle <a} {\scriptstyle <a} { a} {*(yellow) \scriptstyle a+1} }
        * {4,5}
        &&\text{ se $a=b$; } 
        \ytableausetup{aligntableaux=bottom}
    \end{align*}

    Nel primo caso (inserimento regolare) \`e facile verificare che l'inserimento
    di $b$ nella riga successiva avverr\`a in una casella
    debolmente a sinistra della casella contenente $a$, la crescenza delle
    colonne verr\`a preservata e il percorso di inserimento si muover\`a debolmente
    a sinistra.

    Il secondo caso (inserimento speciale) deve essere ulteriormente diviso in due sottocasi:
    se sopra la casella di $a$ c'\`e la lettera $a+1$, allora anche l'inserimento
    di $a+1$ sar\`a speciale e la riga sopra non varia;
    altrimenti, se la lettera sopra la casella di $a$ \`e maggiore di $a+1$,
    si conclude come nel caso dell'inserimento regolare.
    ...
    % preserver\`a la crescenza delle colonne e il percorso di inserimento
    % si muover\`a debolmente a sinistra.

    Per mostrare che $v' = \rowreading(T \leftarrow k) \in \rw(\w)$, basta iterare
    per ogni riga l'Osservazione~\ref{rmk:coxeter_righe}.
    Questo mostra come ottenere $v'$ da $v = \rowreading(T)k$ tramite una serie di 
    operazioni di Coxeter.
\end{proof}

Enunciamo e dimostriamo due lemmi che saranno utili in seguito.
% TODO i due bei lemmi vogliono un esempio?
% NOTE usa direzioni cardinale?
...
\begin{proof}
    Si procede induttivamente, mostrando che in ogni riga l'ordine dei percorsi
    \`e quello giusto e continua a valere la disuguaglianza fra le lettere eventualmente
    spinte.

    Se l'inserimento di $a$ \`e regolare, la lettera $a$ spinge una lettera $c > a$ e
    prende il suo posto.
    L'inserimento di $b$ spinge una lettera $d > b > a$ che \`e anche maggiore di $c$
    perch\'e la casella di $a$ era precedentemente occupata da $c$.

    Se l'inserimento di $a$ \`e speciale, la lettera $a$ spinge $c=a+1$ e il percorso di
    inserimento passa per $a+1$.
    L'inserimento di $b$ spinge una lettera $d > b \geq a+1$ e il percorso passa per la
    casella precedentemente occupata da $d$.
\end{proof}
...
\begin{proof}
    Si procede in modo analogo alla dimostrazione del Lemma~\ref{lem:percorsi_strict}.

    Dopo l'inserimento di $a$, che spinge una lettera $c>a$,
    la lettera $a > b$ compare nella riga.
    Allora, l'inserimento di $b$ spinge una lettera $d$ tale che $b < d \leq a$,
    da cui il percorso passa debolmente a sinistra della casella di $a$ e
    continua a valere la disuguaglianza $c > d$ tra le lettere spinte.
\end{proof}
...
\begin{definition}[$P$-tableau]
    Sia $\w$ una permutazione in $\sym_n$ e sia $v=v_1 v_2 \cdots v_s \in \rw(\w)$ una sua
    parola ridotta. 
    Definisco $P(v)$, ovvero il $P$-tableau associato a $v$, come il tableau
...
Enunciamo alcune propriet\`a fondamentali del $P$-tableau EG,
la cui dimostrazione segue immediatamente dall'iterazione della
Proposizione~\ref{prop:propriet_inserimento}.
...
\begin{proposition}[Propriet\`a del $P$-tableau EG]
    ...
    \end{enumerate}
    % NOTE non mi sembra ci siano. aggiungere eventuali propriet\`a del P-tableau
\end{proposition}

In completa analogia con la corrispondenza RSK, definiamo il $Q$-tableau, o
\emph{recording} tableau, che tiene traccia di dove vengono aggiunte le caselle 
e permette di invertire il processo.

\begin{definition}[$Q$-tableau]
    Sia $v = v_1 v_2 \cdots v_s$ una parola ridotta. 
    Definisco $Q(v)$, ovvero il $Q$-tableau associato a $v$, come il tableau di Young
...
\begin{theorem}{\cite[Theorem~6.25]{edelman-greene-87}}
    ...
    \[
        \varphi_\EG \colon v \mapsto (P(v), Q(v))
    \]
    \`e una bigezione tra:
    \begin{itemize}
...
\begin{proof}
    % TODO MODIFICA qui
    Bisogna dimostrare che, per ogni coppia di tableaux $(P,Q)$ come nell'enunciato
    del teorema,
    \`e sempre possibile trovare un'unica parola ridotta $v\in \rw(\w)$ che restituisce
    $(P,Q)$ tramite la corrispondenza EG.

    % La parola $v$ verr\`a costruita a partire dall'ultima lettera, tramite un
    % algoritmo che, conoscendo la posizione finale dell'ultima lettera inserita,
    % inverte l'

    \`E sufficiente mostrare che, per ogni casella angolare di $P$, \`e sempre possibile
    rimuovere quella casella invertendo un qualche inserimento EG.
    Allora l'ultima lettera di $v$ si pu\`o determinare invertendo l'inserimento
    che corrisponde alla casella di $Q$ con il numero pi\`u alto.
    % invertire l'algoritmo di inserimento EG, conoscendo la posizione dell'ultima casella
    % inserita, l'algoritmo di inserimento EG pu\`o sempre essere invertito in modo univoco,
    % determinando cos\`\i\ l'ultima lettera di $v$.
    In seguito mostriamo come ricavare  questo inserimento inverso.
    Per tutti i dettagli rimandiamo a \cite[Theorem~6.25]{edelman-greene-87}.
...
\begin{definition}
    \label{def:a_w_lambda}
    Per ogni permutazione $\w \in \sym_n$ e partizione 
    $\lambda$, 
    definiamo $a_\w^\lambda$ come il numero di tableaux ridotti di forma $\lambda$
    il cui row-reading \`e una parola ridotta per $\w$.
    In simboli:
    \[
...
In \cite[Section~4]{stanley-84}, Stanley ha dato alcune condizioni necessarie su $\lambda$ 
affinch\'e $a_\w^\lambda$ sia diverso da zero.
...
permutazione di lunghezza massima.

\begin{theorem}
    {\cite[Theorem~6.26]{edelman-greene-87}}
    \label{thm:stanley_og}
    ...
    Young \`e una ``scala'' con $\binom{n}{2}$ caselle.
\end{theorem}
...
\begin{proof}
    ...
    $T$ \`e un tableau ridotto per $\w_0$, allora $T = T_0$.
    \begin{figure}[htbp]
        \centering
        \[
            T_0 = 
    ...
    Per ogni casella $(i,j)$ nel tableau $T$, chiamiamo $t_{ij}$ la lettera
    contenuta in quella casella.
    Utilizzando l'Osservazione~\ref{rmk:tab_ridotto_crescenza} sulla crescenza stretta
    di righe e colonne di un tableau ridotto, otteniamo
    che $t_{ij} \geq i+j-1$.
    Siccome $T$ contiene solo lettere da $1$ a $n-1$, deve valere
    $i+j \le n$ per ogni casella $(i,j)$ di $T$, ovvero la forma di $T$ \`e contenuta
    nella forma ``a scala'' di $T_0$, che ha esattamente $\binom n 2$ caselle.

    Poich\'e $\len(\w_0) = \binom n 2$, il tableau $T$ deve contenere $\binom n 2$ caselle,
    ovvero \emph{tutte} quelle della forma $\lambda_0$ in cui \`e contenuta,
    da cui $\shape(T) = \lambda_0$.
...
Ricordiamo qualche nozione gi\`a definita nella Sezione~\ref{sec:incr_fact}.

Data una permutazione $\w \in \sym_n$, dico che $v = v^1 v^2 \cdots v^s$ \`e una 
...
Chiamiamo $\incrfact(\w)$ l'insieme delle fattorizzazioni crescenti di $\w$.

Per ogni fattorizzazione crescente $v = v^1 v^2 \cdots v^s$ definisco il suo \emph{peso}
$\wt(v)$ come il vettore di interi tale che
$\wt(v)_i = \len(v^i)$ per ogni $i = 1, \dots, s$.
Per ogni peso $\mu$,
chiamo $\incrfact_\mu(\w)$ l'insieme delle fattorizzazioni
crescenti di $\w$ con peso $\mu$.
...
\begin{example}
    ...
    \[
        ...
        \end{array}
    \]
    % NOTE disegno rattoppato
\end{example}

\begin{proposition}
    Per ogni fattorizzazione crescente $v$, il tableau $Q(v)$ definito sopra \`e un 
...
Vale quindi una versione pi\`u generale del Teorema~\ref{thm:eg1}.
Ricordiamo che il peso di un tableau semistandard $T$ \`e il vettore di interi
la cui componente $i$-esima conta quante lettere $i$ compaiono in $T$.

\begin{theorem}
    \label{thm:eg2}
    Fissata una permutazione $\w \in \sym_n$ e un peso $\mu \in \ZZ^\ell$,
    ...
    \`e una bigezione tra:
    \begin{itemize}
        \item l'insieme $\incrfact_\mu(\w)$ delle fattorizzazioni crescenti
            di $\w$ con peso $\mu$;
        \item l'insieme delle coppie $(P,Q)$ di tableaux di Young con la stessa forma,
            tali che $P$ sta in $\redtab(\w)$ e
            $Q$ \`e semistandard con peso $\mu$.
    \end{itemize}
\end{theorem}
...
\begin{proof}
    ...
    a quel fattore sono state aggiunte da quella pi\`u a sinistra a quella pi\`u a destra.
    ...
    Alla fine si suddivide la parola ottenuta in fattori, rispettando il peso di $Q$.
\end{proof}
...
Ricordiamo dalla Definizione~\ref{def:stanley_f_qsimm} e 
dall'Esempio~\ref{ex:schur_simmetriche}:
\begin{align*}
...
\begin{corollary}
    ...
    % o, equivalentemente:
    \begin{equation}
        \label{eq:stan_in_schur}
        \stan_{\w^{-1}} = \sum_{\lambda} a_\w^\lambda \, s_\lambda,
    \end{equation}
    dove $\lambda$ varia fra le partizioni e $a_\w^\lambda$ \`e,
    come da Definizione~\ref{def:a_w_lambda}, il numero
    di tableaux ridotti di forma $\lambda$ il cui row-reading
...
\begin{proof}
    Per ogni intero positivo $\ell$ e per ogni vettore $\mu$ di interi non-negativi in
    $\NN^\ell$, il coefficiente del monomio $\xx^\mu$ in $\stan_{\w^{-1}}$ \`e
    $\lvert \incrfact_\mu(\w)\rvert$, ovvero il
    numero di fattorizzazioni crescenti di $\w$ di peso $\mu$.
    
    Nel termine di destra, $a_\w^\lambda$ conta i tableaux in $\redtab(\w)$ di
    forma $\lambda$ e il coefficiente del monomio $\xx^\mu$ di $s_\lambda$
    conta i tableaux semistandard di peso $\mu$ e stessa forma.
    Sommando per tutte le forme, trovo che i coefficienti del monomio
    $\xx^\mu$ nei due termini
    contano le cardinalit\`a degli insiemi tra cui il Teorema~\ref{thm:eg2}
    fornisce una bigezione.
\end{proof}
...
un isomorfismo di cristalli, ovvero la struttura di cristallo di
Morse--Schilling definita sulle fattorizzazioni
crescenti nella Sezione~\ref{sec:cristallo_morse_schilling} viene mappata nella struttura 
di cristallo sui tableaux semistandard definita nella Sezione~\ref{sec:crys_tableaux}.
...
essere vista come una bigezione tra l'insieme $\incrfact^\ell(\w)$ delle
...
Ricordiamo che nella Sezione~\ref{sec:cristallo_morse_schilling}
abbiamo definito una struttura di cristallo $B(\w)$ di tipo $A_{\ell-1}$ sull'insieme 
$\incrfact^\ell(\w)$.
...
\]
dove $\lambda$ varia tra le partizioni e $a_\w^\lambda$ \`e,
come nella Definizione~\ref{def:a_w_lambda}, il numero di 
tableaux ridotti di forma $\lambda$ il cui row-reading \`e una parola ridotta per $\w$.
...
\begin{theorem}
    ...
    In particolare:
    \begin{enumerate}
        \item \label{item:thm_pt1}
            l'azione di $f_i$ ed $e_i$ sulla fattorizzazione crescente $v$, se definita,
            non cambia il suo $P$-tableau, ovvero
            \begin{align*}
                P(f_i(v)) &= P(v) && \text{se $f_i(v) \neq \zero$,} \\
                P(e_i(v)) &= P(v) && \text{se $e_i(v) \neq \zero$;}
            \end{align*}
        \item \label{item:thm_pt2}
            l'azione di $f_i$ ed $e_i$ sulla fattorizzazione crescente $v$ commuta
...
\begin{proof}
    ...
    \[
        v \approx \rowreading(P(v)) = \rowreading(P(v')) \approx v'.
    \]
...
Ritorniamo ora al cristallo di Morse--Schilling $B(\w)$ per ogni $\w \in \sym_n$.
...
\begin{proof}
    ...
    Vogliamo introdurre opportune modifiche in modo da usare solo operazioni di Coxeter--Knuth.
    ...
    ridotta dell'Esempio~\ref{ex:f_agisce_su_incrfact}:
    % dove la lettera in 
    % grassetto \`e l'ultima lettera non appaiata di $v^1$.
    \[
        v = v^1  v^2 = 
        ( 1, 2, 3, 9, 11 \mid \mathbf{12}, 13, 14 \mid 18 ) \,
        ( 1, 5, 8 \mid 12, 13 \mid 15, 16).
    \]

    % TODO: emoji del teschio + rifai con lem:divisione_sgravo_comparativo
    % oppure te lo fai andarebene cos\`i
    
    Per prima cosa voglio spostare a sinistra le lettere del primo blocco del
    secondo fattore.
    Per il Lemma~\ref{lem:divisione_sgravo_comparativo}, queste lettere
    sono ordinatamente minori delle ultime lettere nel primo blocco del primo
    fattore.
    ...
    \[
        ...
        ( 12, 13 \mid 15, 16).
    \]
    Nel muovere ogni lettera, il primo scambio \`e  della forma in
    \eqref{subeq:ck_dx}, tutti i successivi sono della forma in 
    \eqref{subeq:ck_sx}.

    Nello stesso modo, tutte le lettere del terzo blocco del primo fattore si
    possono spostare a destra, fino alla lettera ordinatamente corrispondente 
    nell'ultimo blocco, ottenendo:
    \[
        % ( 1, 2 \text{-} 1, 3, 9 \text{-} 5,
        ( 1, 2 , 3\text{-} 1, 9 \text{-} 5,
        11 \text{-} 8\mid \mathbf{12}, 13, 14  ) \,
        ( 12, 13 \mid 18 \text{-}15,  16).
    \]

    Poi, si verifica che tutte le operazioni di Coxeter utilizzate nel
    Lemma~\ref{lem:parole_equiv} sono gi\`a operazioni di Coxeter--Knuth, quindi ottengo:
    \[
        % ( \mathbf{1}, 2 \text{-} 1, \mathbf{3}, 9 \text{-} 5,
        ( 1, 2 , 3\text{-} 1, 9 \text{-} 5,
        11 \text{-} 8\mid  13, 14  ) \,
        % ( 12, 13, \mathbf{14} \mid \mathbf{15}, 18 \text{-} 16).
        ( 12, 13, \mathbf{14} \mid 18 \text{-}15,  16).
    \]
    Alla fine, tutte le lettere spostate durante i primi step
    possono ritornare alle loro posizioni originali, ottenendo
    \[
        f_1(v) = 
        ( 1, 2, 3, 9, 11 \mid 13, 14 \mid 18 ) \,
        ( 1, 5, 8 \mid 12, 13, \mathbf{14} \mid 15, 16).
        \qedhere
    \]
\end{proof}
...
\begin{proof}
    ...
    Prima dimostriamo il punto~\ref{item:thm_pt1}.
    ...
    il punto~\ref{item:thm_pt1}.

    Prima di dimostrare il punto~\ref{item:thm_pt2}, introduciamo un po' di notazione.

    Denotiamo le lettere dello $i$-esimo e dello $i+1$-esimo fattore di $v$ con
    \begin{align*}
        v^i &= x_1 x_2 \cdots x_j \cdots x_{\alpha_i}, \\
        v^{i+1} &= y_1 y_2 \cdots y_k \cdots y_{\alpha_{i+1}}.
    \end{align*}
    ...
    lettera non sia stato ``toccato'' dall'inserimento delle altre lettere intermedie.
    ...
    \begin{proof}[Dimostrazione del lemma]
        ...
        del percorso di $x_j$.
        % NOTE forse qui esempio.
    \end{proof}

    Studiamo ora i percorsi di inserimento delle lettere di $v^i$ e come cambiano
    rimuovendo $x_j$.

    Per il Lemma~\ref{lem:percorsi_strict}, per ogni $r = 1, \dots, \alpha_i-1$
        ...
        \begin{tikzpicture}[remember picture]
            ...
            \node (T2) at (6,0) {
                    \ytableaunumber{
                        ...
                        {\tmn{B31}{ 6}} {\tmn{B32}{ 9}} {\tmn{B33}{14}} {\tmn{B34}{15}} {\tmn{B35}{22}} {\tmn{B36}{24}},
    ...
    row-reading del $Q$-tableau in cui le lettere $i$ sono sostituite da parentesi
    ...
    dove per convenzione $a_r = \infty$ se il percorso non raggiunge la riga $r$.

    Nel seguito dimostriamo che la deviazione avviene 
    alla prima riga $r$ in cui $a_{r+1} > b_r$.

    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    % TODO questo \`e di un greve
    ...
    problemi nella dimostrazione (che \`e chiara nel caso senza inserimenti
    speciali).
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[remember picture]
            ...
            \begin{scope}[
                ...
                \draw[grPath]
                    ([yshift=-7pt]A12.south) to
                    (A12.center) to
                    (A13.center) to
                    ([yshift=7pt]A13.north);
                \draw[grPath]
                    ([yshift=-7pt]B12.south) to
                    (B12.center) to
                    ([yshift=7pt]B12.north);
                \draw[grPath]
                    ([yshift=-7pt]A22.south) to
                    (A22.center) to
                    (A23.center) to
                    ([yshift=7pt]A23.north);
                \draw[grPath]
                    ([yshift=-7pt]B22.south) to
                    (B22.center) to
                    ([yshift=7pt]B22.north);
                \draw[grPath]
                    ([yshift=-7pt]A32.south) to
                    (A32.center) to
                    (A33.center) to
                    ([yshift=7pt]A33.north);
                \draw[grPath]
                    ([yshift=-7pt]B32.south) to
                    (B32.center) to
                    ([yshift=7pt]B32.north);
                \node[blNode]
    ...
    $y_{k+1}, y_{k+2}, \dots, y_{\alpha_{i+1}}$, sempre con l'obiettivo di mostrare
    che la casella che viene rimossa quando non si inserisce $x_j$ \`e l'ultima
    parentesi chiusa non appaiata quando scrivo una ``$)$'' alle estremit\`a dei
    percorsi d'inserimento di $v^i$ e una ``$\lparen$'' alle estremit\`a di quelli
    di $v^{i+1}$.
    ...
    % Vediamo prima il caso in cui $x_j < y_{k+1}$ e studiamo a parte 
    % il caso in cui $x_j = y_{k+1}$.
    %
    ...
    \begin{itemize}
        ...
        \item se non c'\`e alcuna buca tra i due percorsi, allora il percorso di
            ...
            debolmente
            % strettamente
            a destra di quello di $x_j$.
    \end{itemize}

    Per il Lemma~\ref{lem:percorsi_weak}, il percorso di $y_{k+1}$ rimane debolmente
    a sinistra di quello di $x_{j+1}$.

    % NOTE forse qui induzione scritta bene
    Studiamo prima l'inserimento senza una buca. La situazione \`e la seguente, con
    $a_2 \leq b$.
    \begin{center}
        \begin{tikzpicture}[remember picture]
            \node (esempi) {
                    \ytableaushort{
                        {\tmn{A13}{a_2}} ,
                        \none {\tmn{A12}{a_1}} {c_1} {c_2} {\cdots} {\tmn{B12}{b}}
                        \none {\none[\leftarrow d]}
                    }
                };
        ...
        \end{tikzpicture}
    \end{center}
    Sia inoltre $\tilde{a}_1$ la lettera il cui inserimento nella riga ha fatto
    passare il percorso di $x_j$ per la casella di $a_1$, ovvero $\tilde{a}_1 = a_1$
    se l'inserimento era regolare, oppure $\tilde{a}_1 = a_1 -1$ se l'inserimento
    \`e stato speciale.
    Definiamo similmente $\tilde{a}_2$.

    Se l'inserimento di $y_{k+1}$ non ha incontrato buche precedentemente, possiamo
    ragionare induttivamente e ipotizzare che $\tilde{a}_1 \leq d $ (che vale al primo
    step perch\'e $x_j \leq y_{k+1}$).

    Siccome prima dell'inserimento di $x_j$ e $x_{j+1}$ la casella di
    $a_1$ era occupata da $\tilde{a}_2$, abbiamo che 
    $\tilde{a}_2 \leq c_1 < c_2 < \cdots$.
    Abbiamo anche che $\tilde{a}_2 \leq a_2 \leq b$.

    Dividiamo in casi:
    \begin{itemize}
        \item se $\tilde{a}_1 = a_1$, la lettera $d$ spinge una $c$ oppure $b$,
            che in ogni caso \`e maggiore o uguale a $\tilde{a}_2$;
        \item se $\tilde{a}_1 = a_1 -1$, allora $\tilde{a}_2 = a_1$, da cui 
            $d \geq \tilde{a}_1$ spinge $a_1$, una $c$ o $b$, che in ogni caso \`e maggiore o uguale
            ad $\tilde{a}_2 = a_1$.
    \end{itemize}

    In ogni caso il nuovo percorso si posiziona debolmente a destra di quello di $x_j$
    e la disuguaglianza tra le due lettere spinte continua a valere alla riga successiva.
    
    Vediamo ora cosa succede alla prima buca che si incontra. La situazione \`e la
    seguente, con $b< a_2$ e, come prima, $\tilde{a}_1 \leq d$.
    \begin{center}
        \begin{tikzpicture}[remember picture]
            \node (esempi) {
                    \ytableaushort{
            ...
            \begin{scope}[
                ...
                \draw[grPath]
                    ...
                    ([yshift=7pt]A13.north);
                \draw[grPath]
                    ([yshift=-7pt,xshift=7pt]B12.south) to
                    (B12.center) to
                    ([yshift=7pt]B12.north);
                \draw[grPath]
                    ([yshift=-7pt]A22.south) to
                    ...
                    ([yshift=7pt]A23.north);
                \draw[grPath]
                    ([yshift=-7pt,xshift=7pt]B22.south) to
                    (B22.center) to
                    ([yshift=7pt]B22.north);
                \draw[blPath]
                    ([yshift=-7pt]B22.south) to
                    (B22.center) to
                    (C23.center) to
                    ([yshift=7pt]C23.north);
            %
            \end{scope}
        \end{tikzpicture}
    \end{center}

    Come visto sopra, se $\tilde{a}_1 = a_1 -1$, allora vale $\tilde{a}_2 = a_1$,
    che \`e assurdo, infatti vale
    \[
        a_1 +1 \leq b < a_2 \leq \tilde{a}_2 +1 = a_1 + 1.
    \]
    Allora $\tilde{a}_1 = a_1$ e $d$ non pu\`o spingere $a_1$.

    Ricordando che il percorso di $y_{k+1}$ deve rimanere debolmente a sinistra
    del percorso di $x_{j+1}$, deve succedere che $d$ spinge $b$, il quale,
    essendo minore di $a_2$, si sposta debolmente a sinistra del percorso di $x_j$.
    Inoltre, il nuovo percorso rimarr\`a definitivamente a sinistra di quello di $x_j$ per 
    il Lemma~\ref{lem:percorsi_weak}.

    La Figura~\ref{fig:tab_con_intertionp_v2} continua l'Esempio~\ref{ex:thm_tab_con_inspath}
    mostrando l'inserimento delle ulteriori lettere
    \[
        y_1 y_2 y_3 y_4 = 5, 7, 12, 15.
    \]
        ...
        \begin{tikzpicture}[remember picture]
            \node (T1) {
                    \ytableaunumber{
                        {\tmn{A61}{15}} {\tmn{A62}{19}},
                        {\tmn{A51}{13}} {\tmn{A52}{18}},
                        {\tmn{A41}{ 8}} {\tmn{A42}{ 9}} {\tmn{A43}{14}} {\tmn{A44}{22}} {\tmn{A45}{24}},
                        {\tmn{A31}{ 6}} {\tmn{A32}{ 7}} {\tmn{A33}{12}} {\tmn{A34}{15}} {\tmn{A35}{16}} {\tmn{A36}{17}} {\tmn{A37}{18}},
                        {\tmn{A21}{ 2}} {\tmn{A22}{ 3}} {\tmn{A23}{ 5}} {\tmn{A24}{ 6}} {\tmn{A25}{ 8}} {\tmn{A26}{13}} {\tmn{A27}{16}},
                        {\tmn{A11}{ 1}} {\tmn{A12}{ 2}} {\tmn{A13}{ 4}} {\tmn{A14}{ 5}} {\tmn{A15}{ 7}} {\tmn{A16}{10}} {\tmn{A17}{12}} {\tmn{A18}{15}}
                    }
                };
            ...
            \end{scope}
        \end{tikzpicture}
        \caption{L'inserimento nel tableau della Figura~\ref{fig:tab_con_insertionp}
        delle ulteriori lettere $5$, $7$, $12$ e $15$.}
        \label{fig:tab_con_intertionp_v2}
    \end{figure}
    
    Abbiamo dunque dimostrato che il percorso di inserimento di $y_{k+1}$
    scavalca quello di $x_j$ alla propria sinistra esattamente in corrispondenza
    della buca tra $x_j$ e $x_{j+1}$ e che, se non c'\`e alcuna buca, non lo scavalca.

    Se lo scavalca, la stessa dimostrazione si pu\`o adattare per il percorso
    successivo ed eventualmente quelli dopo.
    Guardiamo per esempio il percorso di $y_{k+2}$.

    Come prima, poich\'e $y_{k+2}< x_{j+2}$ e il percorso di $x_{j+2}$ non \`e stato toccato 
    dagli inserimenti precedenti, il percorso di $y_{k+2}$ sar\`a debolmente
    a sinistra di quello di $x_{j+2}$.

    Per non intersecare il percorso di $y_{k+1}$, quello di $y_{k+2}$ deve passare
    a \emph{destra} della buca tra $x_j$ e $x_{j+1}$.
    Da quel punto in poi, invece, ci si ritrova nella stessa identica situazione del
    percorso di $y_{k+1}$, per cui si dimostra che il percorso di $y_{k+2}$ scavalca
    quello di $x_{j+1}$ se e solo se c'\`e una buca.

    Per concludere la dimostrazione, scriviamo una parentesi chiusa ``$)$'' alle estremit\`a
    dei percorsi di inserimento delle lettere del fattore $v^i$ e una parentesi
    aperta ``$\lparen$'' a quelle del fattore $v^{i+1}$ e leggiamo da sinistra a destra
    e dall'alto al basso la parola ottenuta.
    Come preannunciato, bisogna dimostrare che l'ultima parentesi chiusa
    non appaiata si trova in corrispondenza dell'ultimo percorso che devia, ovvero
    l'ultimo ad avere una buca immediatamente alla propria sinistra.
    Sia $x_{j+s}$ la lettera a cui corrisponde questo percorso.

    Per esempio, la stringa di parentesi che si legge dal tableau in
    Figura~\ref{fig:tab_con_intertionp_v2} \`e:
    \[
        (\,(\,)\,)\,\underline{)}\,\lparen\,)\,\lparen,
    \]
    in cui l'ultima ``$)$'' non appaiata \`e quella sottolineata.
    Si verifica, confrontando la Figura~\ref{fig:tab_con_insertionp}, che corrisponde
    proprio alla casella che viene rimossa quando non si inserisce $x_j = x_1$.


    Verifichiamo che tutte le parentesi aperte corrispondenti a $y_1, \dots, y_k$
    sono appaiate con parentesi corrispondenti ad alcune tra $x_1, \dots, x_{j-1}$.
    Questo segue rapidamente dalla dimostrazione del Lemma~\ref{lem:thm_trascuro_inizio},
    in cui dimostro che la ``$\lparen$'' di $y_{r}$ compare prima della
    ``$)$'' di $x_{j-k+r-1}$ per ogni $r = 1, \dots , k$.

    Abbiamo gi\`a dimostrato che, per ogni $r = 1, \dots, s$, dato
    che c'\`e una buca tra i percorsi di $x_{j+r-1}$ e $x_{j+r}$, il percorso
    di $y_{k+r}$ scavalca e quindi la sua parentesi aperta compare prima di quella
    chiusa di $x_{j+r-1}$, da cui anche tutte quelle sono appaiate.

    Poich\'e non ci sono buche tra il percorso di $x_{j+s}$ e quello di
    $x_{j+s+1}$, il percorso di $y_{k+s+1}$ \emph{non} scavalca quello di $x_{j+s}$.
    Tutte le parentesi aperte inserite prima di $y_{k+s+1}$ sono appaiate per quanto
    detto prima e tutte quelle che verranno inserite dopo saranno comunque a destra
    di quella di $y_{k+s+1}$.
    Allora la parentesi chiusa associata a $x_{j+s}$ \emph{non} \`e appaiata.

    Verifichiamo infine che tutte le parentesi chiuse corrispondenti a
    $x_{j+s+1}, \dots, x_{\alpha_i}$ sono appaiate con parentesi aperte tra quelle
    di $y_{k+s+1}, \dots, y_{\alpha_{i+1}}$.
    Per ogni $r = s+1, \dots, \alpha_i - j$, per il Lemma~\ref{lem:divisione_sgravo_comparativo}
    vale $ y_{k+r} < x_{j+r} $, da cui prima della parentesi chiusa di $x_{j+r}$
    compare quella aperta di $y_{k+r}$.

    Questo dimostra che l'ultima parentesi chiusa non appaiata \`e proprio quella
    di $x_{j+s}$, ultima lettera il cui percorso devia,
    dimostrando che l'operatore $f_i$ di Kashiwara commuta con
    la mappa $Q$ di Edelman--Greene.

    Per dimostrare il teorema, bisognerebbe ripetere il tutto per dimostrare
    che anche l'operatore $e_i$ commuta, ma qui non lo faremo.
\end{proof}
