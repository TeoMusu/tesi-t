...
\begin{definition}
    Un tableau semistandard si dice \emph{ridotto} se il suo
...
\begin{proof}
    ...
    L'inserimento di $b$ spinge una lettera $d > b \leq a+1$ e il percorso passa per la
...
\begin{corollary}
    ...
    di tableaux semistandard il cui row-reading
    \`e una parola ridotta per $\w$.
    % In simboli:
    % \begin{equation}
    %     \label{eq:def_a_w_lambda}
...
\begin{proof}
    ...
    $\NN^\ell$, il coefficiente del monomio $\xx^\mu$ in $\stan_{\w^{-1}}$ \`e il
    numero di fattorizzazioni crescenti di peso $\mu$.
...
\begin{proof}
    ...
    Poi, si verifica che tutte le operazioni di Coxeter richieste dal Lemma~\ref{lem:parole_equiv}
...
\begin{proof}
    ...
    Successivamente abbiamo dimostrato che, per ogni $r = 1, \dots, s$, dato
    ...
    di $x_{j+s}$, dimostrando che l'operatore $f_i$ di Kashiwara commuta con
